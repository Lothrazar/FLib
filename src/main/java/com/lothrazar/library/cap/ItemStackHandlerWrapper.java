...
public class ItemStackHandlerWrapper implements IItemHandler, IItemHandlerModifiable, INBTSerializable<CompoundTag> {
  ...
  public CompoundTag serializeNBT(HolderLookup.Provider p) {
    ...
    cmp.put(NBT_INPUT, input.serializeNBT(p));
    cmp.put(NBT_OUTPUT, output.serializeNBT(p));
    return cmp;
  }

  @Override
  public void deserializeNBT(HolderLookup.Provider p, CompoundTag nbt) {
    input.deserializeNBT(p,nbt.getCompound(NBT_INPUT));
    output.deserializeNBT(p,nbt.getCompound(NBT_OUTPUT));
  }
