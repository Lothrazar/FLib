...
public class RenderResizableCuboid {
  ...
  public void renderCube(Model3D cube, PoseStack matrix, VertexConsumer buffer, int argb, int light) {
    float red = RenderBlockUtils.getRed(argb);
    float green = RenderBlockUtils.getGreen(argb);
    float blue = RenderBlockUtils.getBlue(argb);
    float alpha = RenderBlockUtils.getAlpha(argb);
